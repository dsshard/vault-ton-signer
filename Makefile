...
build-linux-release:  ## - build a static release linux elf(binary)
	@ CGO_ENABLED=0 GOOS=linux GOARCH=arm64 $(GO) build -ldflags='-w -s -extldflags "-static"' -a -o "$(GOBIN)/release/linux/vault-ton-signer-$(version)" cmd/plugin/*.go
	@ ls -lah $(GOBIN)/release/linux/vault-ton-signer-$(version)
	@ shasum -a 256 $(GOBIN)/release/linux/vault-ton-signer-$(version)
	@ mkdir -p ./.build/vault/plugins
	@ cp $(GOBIN)/release/linux/vault-ton-signer-$(version) ./.build/vault/plugins/vault-ton-signer-$(version)
...
build: build-common ## - build a debug binary to the current platform (windows, linux or darwin(mac))
	@ echo cleaning...
	@ rm -f $(GOBIN)/debug/$(OS)/vault-ton-signer
	@ echo building...
	@ $(GO) build -tags dev -o "$(GOBIN)/debug/$(OS)/vault-ton-signer" cmd/plugin/*.go
	@ ls -lah $(GOBIN)/debug/$(OS)/vault-ton-signer
	@ shasum -a 256 $(GOBIN)/debug/$(OS)/vault-ton-signer

.PHONY: scan
scan: ## - execute static code analysis
	@ gosec -fmt=sarif -out=vault-vault-ton-signer.sarif -exclude-dir=test -exclude-dir=bin -severity=medium ./... | 2>&1
	@ echo ""
	@ cat $(path)/vault-vault-ton-signer.sarif
